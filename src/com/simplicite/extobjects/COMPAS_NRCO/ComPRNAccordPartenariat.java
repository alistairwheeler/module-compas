...
/**
 * External object ComPRNAccordPartenariat
 */
public class ComPRNAccordPartenariat extends ExternalObject {
	...
	public Object display(Parameters params) {
		...
		String templateAccordShortID = (new JSONObject(getGrant().getSystemParam("NRCO_JSREPORT_TPL_SHORTID"))).getString("accordPRN");


        TreeView treeview = getGrant().getTreeView("ComACPNR");
        String reponse = JSONToolHelper.getObjectAsJsonTreeview(getGrant().getTmpObject("ComACPNR"), params.getParameter("id"), treeview);
		JSONObject jsonRep = new JSONObject(reponse);

        AppLog.info(getClass(), "method", reponse, getGrant());

        ObjectDB ac = getGrant().getTmpObject("ComACPNR");
        ac.resetValues();
        ac.resetFilters();
        // le filtre
        ...
        String res = jsreport.report(templateAccordShortID, jsonRep);
