...
/**
 * External object ComBCPbr
 */
public class ComBCPbr extends ExternalObject {
	...
	public Object display(Parameters params) {
		...
		try{
			...
			ObjectDB ac = getGrant().getTmpObject("ComACPBR");
			ac.resetValues();
			ac.resetFilters();
			
			ac.setFieldFilter("comACid", params.getParameter("id"));

			AppLog.info(getClass(), "display", "comComACid "+params.getParameter("id"), getGrant());
			...
			if(!Tool.isEmpty(rows)){
				for(String[] row: rows){
					...
					TreeView treeview = getGrant().getTreeView("ComACPBR");
			
					String reponse = JSONToolHelper.getObjectAsJsonTreeview(getGrant().getTmpObject("ComACPBR"),  params.getParameter("id"), treeview);
					...
					if("EPO".equals(com.getFieldValue("comComTypeLivraison"))){
						...
						jCom.put("line1", "1".equals(com.getFieldValue("comComFichier")) ? com.getFieldDisplayValue("comComSiFichier") : "Sans fichier");
					...
					if((vprivItem.optDouble("comPNRfraisLocAdr"))==Double.NaN){
						...
						AppLog.info(getClass(), "method", String.valueOf(vprivItem.optDouble("comPNRfraisLocAdr", 0)) + " Not Cool NAN", getGrant());
					}
					
					
					
					frais.put("fraisLocAdr", (String.valueOf(vprivItem.optDouble("comPNRfraisLocAdr", 0))).replace(".", ","));
					frais.put("comPNRfraisTraitFich", (String.valueOf(vprivItem.optDouble("comPNRfraisTraitFich", 0))).replace(".", ","));
					frais.put("comPNRfraisDistribStreet", (String.valueOf(vprivItem.optDouble("comPNRfraisDistribStreet", 0))).replace(".", ","));
					frais.put("comPNRfraisLocCar", (String.valueOf(vprivItem.optDouble("comPNRfraisLocCar", 0))).replace(".", ","));
					frais.put("comPNRfraisAnmin", (String.valueOf(vprivItem.optDouble("comPNRfraisAnmin", 0))).replace(".", ","));
					frais.put("comPNRfraisAutre", (String.valueOf(vprivItem.optDouble("comPNRfraisAutre", 0))).replace(".", ","));
					
					//(String.valueOf(vprivItem.optDouble("comPNRfraisAnmin"))).repalce(".", ","))
					
					frais.put("comPNRtotalRemise", (String.valueOf(vprivItem.optDouble("comPNRtotalRemise", 0))).replace(".", ","));
					frais.put("comPNRprixTtc", (String.valueOf(vprivItem.optDouble("comPNRprixTtc", 0))).replace(".", ","));
					frais.put("comPNRtauxRemise", (String.valueOf(vprivItem.optDouble("comPNRtauxRemise", 0))).replace(".", ","));
					...
					jsonRep.put("totalHTprestaCom", String.valueOf(totalPresta).replace(".",","));
					
					JSONObject recap = new JSONObject();
					recap.put("tP", String.valueOf(totalPrestaArrondi * 1.20).replace(".",","));
					recap.put("tJ", String.valueOf(totalJournaux).replace(".",","));
					recap.put("total", String.valueOf(totalArrondi).replace(".",","));
			...
			wp.setReady("ComBCPbr.render('"+wp.getRoot()+"');");
			return wp.toString();
